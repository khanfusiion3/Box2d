...
(function() {
  ...
  DynamicTreeBroadPhase.prototype.UpdatePairs = function(callback) {
    ...
    var i = 0,
      queryProxy;
    for (i = 0; i < __this.moveBuffer.length; ++i) {
...
(function() {
  ...
  Body.prototype.SetPositionAndAngle = function(position, angle) {
    ...
    var broadPhase = this.world.contactManager.broadPhase;
    for (f = this.fixtureList; f; f = f.next) {
  ...
  Island.prototype.Initialize = function(bodyCapacity, contactCapacity, jointCapacity, allocator, listener, contactSolver) {
    ...
    for (i = this.bodies.length; i < bodyCapacity; i++)
      this.bodies[i] = null;
    for (i = this.contacts.length; i < contactCapacity; i++)
      this.contacts[i] = null;
    for (i = this.joints.length; i < jointCapacity; i++)
      this.joints[i] = null;
  }
  ...
  World.prototype.DrawDebugData = function() {
    ...
    if (flags & DebugDraw.e_shapeBit) {
      for (b = this.bodyList; b; b = b.next) {
        xf = b.xf;
    ...
    if (flags & DebugDraw.e_jointBit) {
      for (j = this.jointList; j; j = j.next) {
        this.DrawJoint(j);
      }
    }
    ...
    if (flags & DebugDraw.e_aabbBit) {
      ...
      for (b = this.bodyList; b; b = b.GetNext()) {
        if (b.IsActive() == false) {
          continue;
        }
  ...
  World.prototype.SolveTOI = function(step) {
    ...
    var c;
    for (c = this.contactList; c; c = c.next) {
    ...
    for (;;) {
      ...
      while (queueSize > 0) {
        ...
        for (cEdge = b.contactList; cEdge; cEdge = cEdge.next) {
...
(function() {
  ...
  GravityController.prototype.Step = function(step) {
    ...
    var f = null;
    if (this.invSqr) {
      for (i = this.bodyList; i; i = i.nextBody) {
        body1 = i.body;
        p1 = body1.GetWorldCenter();
        mass1 = body1.GetMass();
        for (j = this.bodyList; j != i; j = j.nextBody) {
          body2 = j.body;
